...
static bool advertisement_report_contains_service_and_name(uint16_t service, std::string str, uint8_t *advertisement_report)
{
        ...
        case TC_W4_SERVICE_RESULT:
            switch(hci_event_packet_get_type(packet))
            {
                ...
                case GATT_EVENT_QUERY_COMPLETE:
                    ...
                    gatt_client_discover_characteristics_for_service_by_uuid16(handle_gatt_client_event, connection_handle, &server_service, ORG_BLUETOOTH_CHARACTERISTIC_CLIENT_SUPPORTED_FEATURES);
                    break;
                default:
                    break;
            }
...
static void hci_event_handler(uint8_t packet_type, uint16_t channel, uint8_t *packet, uint16_t size)
{
    ...
    switch(event_type){
        ...
        case GAP_EVENT_ADVERTISING_REPORT:
            ...
            if (!advertisement_report_contains_service_and_name(ORG_BLUETOOTH_SERVICE_GENERIC_ATTRIBUTE, "PCON" , packet)) return;
            ...
            gamepad_manager.CreateGamepad(bd_addr_to_str(server_addr));
            break;
        case HCI_EVENT_LE_META:
            ...
            switch (hci_event_le_meta_get_subevent_code(packet))
            {
                case HCI_SUBEVENT_LE_CONNECTION_COMPLETE:
                    ...
                    gatt_client_discover_primary_services_by_uuid16(handle_gatt_client_event, connection_handle, ORG_BLUETOOTH_SERVICE_GENERIC_ATTRIBUTE);
                    break;
                default:
                    break;
            }
        ...
        case HCI_EVENT_DISCONNECTION_COMPLETE:
            ...
            if(gamepad_manager.HasGamepad(bd_addr_to_str(server_addr)))
                gamepad_manager.DeleteGamepad(bd_addr_to_str(server_addr));
...
static void heartbeat_handler(struct btstack_timer_source *ts)
{
    ...
    // Restart timer
    btstack_run_loop_set_timer(ts, LED_QUICK_FLASH_DELAY_MS);
