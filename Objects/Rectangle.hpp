...
class Rectangle : public Shape
{
    ...
    Rectangle(EntityManager* manager, uint8_t border, uint8_t fill, bool center, bool visible = false, uint8_t layer = 0, 
        uint8_t rotation = 0, vec2<int> position = {0, 0}, vec2<int> size = {0, 0}) 
        : Shape(manager, SHAPE::RECTANGLE, visible, layer, rotation, position, size)
    {
        this->data[1] = border;
        this->data[2] = fill;
        this->data[3] = center;
